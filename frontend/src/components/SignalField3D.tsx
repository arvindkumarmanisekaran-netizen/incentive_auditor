  ...
  return (
    <div className="signal-field-3d" aria-hidden="true">
      <svg viewBox="0 0 300 90" preserveAspectRatio="xMidYMid meet">
        ...
        {nodes.map((node) => (
          <g key={node.x}>
            <motion.circle className="signal-network-halo" cx={node.x} cy={node.y} r="7" animate={reducedMotion ? undefined : { r: [6, 11, 6], opacity: [.08, .2, .08] }} transition={{ duration: 3, delay: node.delay, repeat: Infinity }} />
