...
async function askCopilot(page: Page, question: string, caption: string) {
  ...
  await expect
    .poll(() => page.locator(".chat-agent-message").count(), { timeout: 90_000 })
...
test("record complete Incentive Auditor hackathon demo in 1080p", async ({ browser }, testInfo) => {
  ...
  try {
    ...
    await subtitle(
      page,
      "Incentive Auditor — AI-powered anomaly detection and incentive payout validation for Life Sciences field representatives.",
      1700,
    );

    await subtitle(
      page,
      "Opening an isolated PostgreSQL workspace for the demo user. Existing users return to their own governed dataset.",
      900,
    );
    ...
    await subtitle(
      page,
      "The Analysis workspace combines deterministic anomaly analytics with a live multi-agent investigation workflow.",
      1400,
    );
    ...
    await expect
      .poll(() => representativeSelect.locator("option").count(), { timeout: 30_000 })
      .toBeGreaterThan(1);
    ...
    await subtitle(
      page,
      "Selecting a field representative and the July 2026 investigation period before launching the audit.",
      1000,
    );
    ...
    await subtitle(
      page,
      "The agentic workflow is now planning the investigation and evaluating Sales/Rx, Doctor/Territory, Payout, Risk, Summary and Peer evidence.",
      1700,
    );
    ...
    await subtitle(
      page,
      "Investigation complete — the final risk synthesis is reconciled with the underlying analytical evidence.",
      1300,
    );

    // ---------------------------------------------------------------------
    // 3. Top investigation dashboard and every visible chart
    // ---------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------
    // 4. Investigation Evidence tabs — every section and every chart tooltip
    // ---------------------------------------------------------------------
    ...
    await subtitle(
      page,
      "Investigation Evidence provides drill-down views across products, peers, doctor/territory behaviour and historical trends.",
      1200,
    );
    ...
    // Return to product analysis for the final analysis view.
    await workspace.getByRole("button", { name: "Sales & Products", exact: true }).click();
    await pause(500);

    // ---------------------------------------------------------------------
    // 5. AI Copilot — contextual explanation + governed DB query
    // ---------------------------------------------------------------------
    ...
    await subtitle(
      page,
      "Opening Investigation Copilot. It receives the current investigation context and keeps database access read-only.",
      1100,
    );

    await askCopilot(
      page,
      "Explain the current finding and tell me why this payout is risky.",
      "Asking Copilot to explain the current anomaly using the selected representative's investigation evidence.",
    );

    await askCopilot(
      page,
      "Show active representatives",
      "Using Copilot's governed read-only query path to retrieve active representative records.",
    );
    ...
    await subtitle(
      page,
      "Data Control centralizes governed database management, synthetic data generation and document ingestion.",
      1200,
    );
    ...
    for (const sectionName of databaseSections) {
      ...
      await subtitle(
        page,
        `Database Manager — browsing ${sectionName} with paginated, selectable, editable governed records.`,
        650,
      );
    }

    // ---------------------------------------------------------------------
    // 7. Edit a database record and save it
    // ---------------------------------------------------------------------
    ...
    await subtitle(
      page,
      "Editing a representative record. The edit form opens as a true modal and blocks interaction with the background.",
      950,
    );
    ...
    await subtitle(
      page,
      "Primary keys remain protected while editable fields and governed status values can be changed.",
      850,
    );
    ...
    if (await editableText.isVisible().catch(() => false)) {
      ...
      const updatedValue = currentValue.endsWith(" Demo") ? currentValue : `${currentValue} Demo`;
      await editableText.fill(updatedValue);
      await pause(550);
    }
    ...
    // ---------------------------------------------------------------------
    // 8. Generate synthetic test data and download the package
    // ---------------------------------------------------------------------
    ...
    await subtitle(
      page,
      "Generating a downloadable synthetic Life Sciences dataset for representatives, doctors, products, sales, prescriptions and payouts.",
      1000,
    );
    ...
    await subtitle(
      page,
      "The generator streams progress while creating structured CSV, JSON, XLSX and DOCX test datasets.",
      1300,
    );
    ...
    await subtitle(
      page,
      "Document Processing accepts CSV, JSON, XLSX and DOCX and automatically classifies, maps and validates incoming data.",
      1000,
    );

    const generatedRepresentativeCsv = path.resolve(
      process.cwd(),
      "../backend/app/synthetic/output/representatives.csv",
    );

    const fileInput = documentCard.locator('input[type="file"]');
    await subtitle(
      page,
      "Importing the generated representatives CSV into the same workspace to demonstrate duplicate-key detection.",
      750,
    );
    await fileInput.setInputFiles(generatedRepresentativeCsv);

    await expect(documentCard.locator(".document-results")).toBeVisible({ timeout: 120_000 });
    await subtitle(
      page,
      "The ingestion pipeline has classified the document, mapped its columns, validated records and detected duplicates before any database write.",
      1200,
    );
    ...
    await subtitle(
      page,
      "Duplicate records are reviewed before commit; each conflict can keep the existing row or replace it with the uploaded version.",
      1100,
    );
    ...
    await subtitle(
      page,
      "Applying Replace Existing to the detected duplicate records for this demonstration.",
      900,
    );
    ...
    await subtitle(
      page,
      "Confirming the governed duplicate-resolution actions. No replacement occurs until this explicit confirmation.",
      850,
    );
    await confirmActions.click();

    await expect(documentCard.getByText("Applying confirmed actions...", { exact: false })).toBeVisible({
      timeout: 20_000,
    });
    await expect(documentCard.getByText("Applying confirmed actions...", { exact: false })).toBeHidden({
      timeout: 120_000,
    });
    await subtitle(
      page,
      "Confirmed replacements were applied and Database Manager was refreshed with the imported data.",
      1000,
    );

    // ---------------------------------------------------------------------
    // 10. Verify imported data through Database Manager
    // ---------------------------------------------------------------------
    await databaseCard.getByRole("button", { name: "Representatives", exact: true }).click();
    ...
    await subtitle(
      page,
      "Returning to Representatives verifies the post-import workspace state after duplicate resolution.",
      950,
    );

    // ---------------------------------------------------------------------
    // 11. Finish on AI analysis dashboard
    // ---------------------------------------------------------------------
    await page.evaluate(() => window.scrollTo({ top: 0, behavior: "smooth" }));
    await pause(400);
    ...
    await subtitle(
      page,
      "Incentive Auditor brings anomaly detection, explainable evidence, agentic investigation, governed data operations and AI assistance into one audit workflow.",
      1900,
    );
  } finally {
    await context.close();
