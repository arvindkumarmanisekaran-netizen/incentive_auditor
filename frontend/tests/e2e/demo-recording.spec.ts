...
function demoStatus(message: string) {
...
async function tourCharts(page: Page, root: Locator, sectionLabel: string) {
  ...
  demoStatus(`${sectionLabel}: ${count} chart element(s) found`);
...
test("record complete Incentive Auditor hackathon demo in 1080p", async ({ browser }, testInfo) => {
  demoStatus("Starting 1920x1080 guided demo recording");
  ...
  try {
    ...
    demoStatus("STEP 1/11 — Login and open Arvind workspace");
    await page.goto("/");
    ...
    demoStatus("STEP 2/11 — Run multi-agent investigation");
    ...
    await withHeartbeat("Waiting for multi-agent investigation", async () => {
    ...
    demoStatus("STEP 3/11 — Tour dashboard findings and charts");
    ...
    demoStatus("STEP 4/11 — Tour Investigation Evidence tabs");
    ...
    // 5. AI Copilot
    demoStatus("STEP 5/11 — Demonstrate AI Investigation Copilot");
    ...
    demoStatus("STEP 6/11 — Browse Data Control and Database Manager");
    ...
    for (const sectionName of [
      ...
      demoStatus(`Database Manager — opening ${sectionName}`);
    ...
    demoStatus("STEP 7/11 — Edit and save a representative record");
    ...
    demoStatus("STEP 8/11 — Generate synthetic Life Sciences data");
    ...
    demoStatus("STEP 9/11 — Upload generated records and detect duplicates");
    ...
    await withHeartbeat("Processing uploaded representative records", async () => {
    ...
    // 10. Selectively resolve duplicates and confirm
    demoStatus("STEP 10/11 — Replace selected duplicates and confirm actions");
    ...
    for (let index = 0; index < rowsToReplace; index += 1) {
      ...
      demoStatus(`Duplicate ${index + 1}/${rowsToReplace} — selecting Replace Existing`);
    ...
    const applyingWasVisible = await applyingActions
      .waitFor({ state: "visible", timeout: 3_000 })
      .then(() => true)
      .catch(() => false);

    if (applyingWasVisible) {
      demoStatus("Transient confirmation progress state captured");
      ...
      demoStatus("Confirmation completed before the transient progress message could be captured — continuing with stable completion check");
    }

    await withHeartbeat("Applying confirmed duplicate actions", async () => {
      await expect(documentCard.locator(".document-results")).toBeHidden({ timeout: 120_000 });
    });
    await subtitle(page, "The confirmed duplicate resolutions were applied and Database Manager was refreshed.", 850);

    // 11. Verify imported data and finish on analysis dashboard
    demoStatus("STEP 11/11 — Verify imported data and finish recording");
    await humanClick(page, databaseCard.getByRole("button", { name: "Representatives", exact: true }), 220);
    ...
    demoStatus("✓ Guided demo flow complete — closing browser and saving video");
  } catch (error) {
    testFailure = error;
    console.error(`[DEMO ${elapsed()}] ✗ Demo failed`, error);
  } finally {
    demoStatus("Saving recorded video...");
    await context.close();
    ...
    if (video) {
      ...
      demoStatus(`✓ Video saved: ${finalVideo}`);
    }
  }
