...
from ..analytics.sales_anomaly import calculate_sales_deviation
...
@router.get("/sales-prescription-mismatch")
def sales_prescription_mismatch(
    ...
    query = text("""
        WITH monthly_sales AS (
            ...
            WHERE
                s.status = 'Valid'
                AND rda.representative_id = :representative_id
        ...
        monthly_prescriptions AS (
            SELECT
                rda.representative_id,
                p.product_id,
                DATE_TRUNC('month', p.prescription_date)::date AS month,
                SUM(p.quantity) AS prescription_quantity
            ...
            GROUP BY
                rda.representative_id,
                p.product_id,
                DATE_TRUNC('month', p.prescription_date)::date
        )
    ...
    if not rows:
        return {
            "message": "No sales or prescription data found"
        }
    ...
    if current_row is None:
        return {
            "message": "No data found for requested month"
        }
    ...
    historical_sales_average = (
        sum(float(row.sales_amount) for row in historical_rows)
        / len(historical_rows)
    )

    historical_prescription_average = (
        sum(float(row.prescription_quantity) for row in historical_rows)
        / len(historical_rows)
    )

    analysis = calculate_sales_prescription_mismatch(
        current_sales=float(current_row.sales_amount),
...
def sales_deviation(
    ...
    sales_by_month = {
        row.sales_month: float(row.total_sales)
        for row in rows
    }
    ...
    target_date = next(
        (
            date_value
            for date_value in sales_by_month
            if str(date_value) == target_month
        ),
        None,
    )
    ...
    if target_date is None:
        return {
            ...
            "product_id": product_id,
            "month": month,
            "message": "No sales found for requested month",
        }

    current_sales = sales_by_month[target_date]

    previous_sales = [
        amount
        for date_value, amount in sales_by_month.items()
...
@router.get("/doctor-concentration")
def doctor_concentration(
    representative_id: str,
    product_id: str,
    month: str = Query(
        ...,
...
@router.get("/cross-territory")
def cross_territory_analysis(
    representative_id: str,
    product_id: str,
    month: str = Query(
        ...,
